...
class Mixed_OP(nn.Module):
    ...
    def forward(self, x, rngs):
        #print(rngs)
        return sum([x if i < 0 else self._ops[i](x) for i in rngs])
...
class SuperNetwork(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1.):
        ...
        for t, c, n, s in self.interverted_residual_setting:
            ...
            for i in range(n):
                if i == 0:
                    self.features.append(Mixed_OP(input_channel, output_channel, s))
                else:
                    self.features.append(Mixed_OP(input_channel, output_channel, 1))
    ...
    def forward(self, x, rngs=None):
        ...
        for i, mix_op in enumerate(self.features):
            x = mix_op(x, rngs[i])
