...
class SearchSpace1(SearchSpace):
    ...
    def generate_adjacency_matrix_without_loose_ends(self, upscale=True):
        for adjacency_matrix in self._generate_adjacency_matrix(adjacency_matrix=np.zeros([self.num_intermediate_nodes + 2, self.num_intermediate_nodes + 2]),node=self.num_intermediate_nodes + 1):
            if upscale:
