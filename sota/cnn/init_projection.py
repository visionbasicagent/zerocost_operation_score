...
def project_op(model, input, target, args, cell_type, selected_eid=None):
    ''' operation '''
    #### macros
    num_edges, num_ops = model.num_edges, model.num_ops
    candidate_flags = model.candidate_flags[cell_type]
    ...
    for step, (input, target) in enumerate(proj_queue):
        if epoch < model.num_edges:
            ...
            if args.edge_decision == 'global_op_greedy':
                ...
                selected_eid_normal, best_opid_normal = project_op(model, input, target, args, cell_type='normal')
            ...
            if args.edge_decision == 'global_op_greedy':
                ...
                selected_eid_reduce, best_opid_reduce = project_op(model, input, target, args, cell_type='reduce')
